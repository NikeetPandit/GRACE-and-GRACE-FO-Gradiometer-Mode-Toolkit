...
% find_SCA1B_gaps accepts time-tag of interpolated SCA1B data and returns 
...
%
%   Inputs:
%   (1) Mode: either 1 or 2, representing 1 or 2 mode gradient operation. 
...
%       measurements with at most 6 hours of padding on either side!
%
...
[y, m, d] = ymd(mean(timeGPS2dt(TimeTag(:,1)))); 
Date = datetime(y, m, d); % build a datetime object

%--- See if padding on either side of requested date > 6 hours 
first_timetag = abs(timeGPS2dt(TimeTag(1,1)) - Date) > hours(6); 
last_timetag = abs(timeGPS2dt(TimeTag(end,1)) - (Date + days(1) - seconds(1))) > hours(6);
...
switch Mode 
    ...
    case 2
        ...
        mission = det_GRACEmission(timeGPS2dt(TimeTag(1))); 

        switch mission 
            case "GRACE"
                SCA1B_A = read_SCA1B("A", Date, PathData, 'pad', 8);  % GRACE is selected
                SCA1B_B = read_SCA1B("B", Date, PathData, 'pad', 8); 
            otherwise
        
                SCA1B_A = read_SCA1B("C", Date, PathData, 'pad', 8);  % GRACE_FO is selected
                SCA1B_B = read_SCA1B("D", Date, PathData, 'pad', 8);   
        end
    ...
    case 1
        ...
        SCA1B = read_SCA1B(ID, Date, PathData, 'pad', 8);
...
[~, m] = size(TimeTag); ind_cell = cell([1,m]); 
