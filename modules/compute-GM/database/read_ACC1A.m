function [data, ind] = read_ACC1A(ID, Date, PathA, PathB, varargin)
% READ_ACC1A reads in ACC1A linear accelerations for GRACE-FO. 
%
...
file = strcat(PathA, "\", "ACC1A_", day, "_", ID, "_", "04.txt");
...
fclose(fid);
    ...
    % --- Padding requested date before and after
        for i = 1:3
            data{i} = read_ACC1A(ID, Date + days(i-2), PathA, PathB); 
        end
    ...
    %--- See warning message below
        ...
        data = read_ACC1A(ID, Date, PathA, PathB); 
    end
